...
exports.toEscape = function (value) {
	return this.add(function (value) {
		return Validator.escape(value);
	});
};

exports.toUnescape = function (value) {
	return this.add(function (value) {
		return Validator.unescape(value);
	});
};

exports.toLtrim = function (value, chars) {
	return this.add(function (value) {
		return Validator.ltrim(value, chars);
	});
};

exports.toNormalizeEmail = function (value, options) {
	return this.add(function (value) {
		return Validator.normalizeEmail(value, options);
	});
};

exports.toRtrim = function (value, chars) {
	return this.add(function (value) {
...
exports.toMoment = function (options) {
	return this.add(function (value, source, locals) {
		...
		} else if (options && options.from && isFunction(options.from)) {
			tzFrom = options.from(locals);
		} else if (options && options.from && isString(options.from)) {
			tzFrom = options.from;
		}

		if (options && options.to && isFunction(options.to)) {
			tzTo = options.from(locals);
		} else if (options && options.to && isString(options.from)) {
			tzTo = options.to;
		}
		...
		if (isDate(value)) {
			...
			//https://stackoverflow.com/a/23376374/2620505
			return Moment.tz(new Date(value), tzFrom).tz(tzTo);
		}
	});
};
...
exports.isArrayLength = function (from, to) {
	return this.add(function (arr) {
		if (value.length < from) {
			return {
				error: message || e.message || "%s is too short"
			};
		}
		if (value.length > to) {
			return {
				error: message || e.message || "%s is too long"
			};
		}
		return {
			valid: true
...
exports.custom = function (func, message) {
	...
	if (func.length === 4) {
		...
		return this.add(function (value, source, locals) {
			...
			try {
				var result = func(value, source, locals);
			} catch (e) {
				return {
